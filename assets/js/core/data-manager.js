...
export const getPhotographersData = async () => {
  ...
  try {
    ...
    const data = response && response.ok ? await response.json() : (() => { throw new Error(`Failed to fetch data from ${path}`); })();
    const photographers = data.photographers;
    if (!Array.isArray(photographers)) throw new Error("Invalid data structure: photographers array not found");
    photographersCache = photographers;
    console.log(`Successfully loaded ${photographers.length} photographers`);
